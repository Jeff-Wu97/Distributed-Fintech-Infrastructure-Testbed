...
public class Schnorr {
    ...
    public static HashMap<String, String> ecPara=new HashMap<String, String>();
...
/*
    public static void keyGen() throws Exception  {
        System.out.println("Schnorr Initialize");
        ...
        if (str_pubkeyY_bi.substring(str_pubkeyY_bi.length()-1).equals("1")) {
            pubkey="03"+str_pubkeyX;
        }else {
            pubkey="02"+str_pubkeyX;
        }
        ecPara.put("publicKey",pubkey);
    }
*/
    static String sign(String data) {
        ...
        try {
            ...
            msg = md5.digest();
            BigInteger privateKey= new BigInteger(SystemInfo.privateKey.get(""+SystemInfo.node_id),16);
            System.out.println(privateKey);
            String sig=SystemInfo.node_id+":"+bytesToHex(schnorr_sign(msg, privateKey));
            System.out.println("--- signature generated"+sig);
            return sig;
    ...
    static boolean checkSign(String data, String sig_seg){
        try {
            byte[] msg;
            MessageDigest md5;
            md5 = MessageDigest.getInstance("SHA-256");
            md5.update(data.getBytes());
            msg = md5.digest();
            String[] sig_arr=sig_seg.split(":");
            String node_name=sig_arr[0];
            String sig=sig_arr[1];
            String keyStore="";   //UtilClass.findInTxt("key.txt","KEYof"+node_name).split(",")[0];
            /*
            for (String key:SystemInfo.key_lib){
                if (key.contains("KEYof"+node_name)){
                    keyStore=key;
                }
            }*/
            keyStore=SystemInfo.publicKey.get(sig_arr[0]);
            System.out.println(sig_arr[0]);
            System.out.println(keyStore);
            byte[] pubkey=hexStringToByteArray(keyStore);
            try {
                boolean result_actual = schnorr_verify(msg, pubkey, hexStringToByteArray(sig));
                if (result_actual) {
                    System.out.println("-- verified"+" from"+sig_arr[0]+" "+data);
                    return true;
                }else {
                    System.out.println("--验VerErr1: failed verification");
                    return false;
                }
            }catch(Exception e){
                System.out.println("-- VerErr2: failed verification");
                return false;
            }
        }catch (NoSuchAlgorithmException e){
            e.printStackTrace();
...
/*
    public static void main(String[] args) {
        ///////////////////////////////////////////////////////////////////////////////////
        //产generate the public and privacy key
        try {
            System.out.println("Schnorr: generating key pair");
            keyGen();
            System.out.println("private key generated"+ecPara.get("privateKey"));
            System.out.println("public key generated"+ecPara.get("publicKey"));
        } catch (Exception e) {
