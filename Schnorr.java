...
public class Schnorr {
	...
	private static HashMap<String, String> ecPara=new HashMap<String, String>();
	...
	public static void keyGen() throws Exception  {
        ...
        //公钥压缩方法：若y是奇数，则压缩公钥为03+x；若y是偶数，则压缩公钥为02+x
        String pubkey="";
	...
	public static void main(String[] args) {
		...
		try {
			...
			keyGen();
			System.out.println("已生成私钥："+ecPara.get("privateKey"));
			System.out.println("已生成公钥："+ecPara.get("publicKey"));
		} catch (Exception e) {
		...
		System.out.println("签名的消息："+data+" ，定义在第273行");
		byte[] msg;
		MessageDigest md5;
		try {
			...
			System.out.println("已生成签名："+sig);
			//验证数字签名
			System.out.println("正在验证签名");
			...
			try {
				...
				if (result_actual) {
					...
					System.out.println("验证失败，请检查密钥是否更新");
				}
			}catch(Exception e){
				System.out.println("签名错误，验证失败");
			}
