    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.button_send:
                ...
                TestScript testScript=new TestScript(1*1000,100,1000);
                testScript.start();
                break;
            default:
                break;
                ...
                while (true) {
                    ...
                    while ((info = bufferedReader.readLine()) != null && info != "") {
                        ...
                        if (!SystemInfo.test_off){
                            ...
                            System.out.println("receive len: "+message.length());
                            String[] message_seg=message.split("<|,|>");
                            System.out.println("seg len: ");
            ...
            synchronized (SystemInfo.logTransaction){
                 ...
                 */
                    ...
                    if (verify_result) {
                        ...
                        if(UtilClass.isTimeToSleepInViewChangeTest()){
                            System.out.println("I need sleep for view change check");
                            return;
                        }
                 ...
                 */
                    ...
                    if (verify_result){
                        ...
                        //if (SystemInfo.reqhash_localcommit.containsKey(seg[3])) {
                    ...
                    if (verify_result) {
                        if(UtilClass.isTimeToSleepInViewChangeTest()){
                            SystemInfo.viewChangeTestPrimaryActionFlag++;
                            if (SystemInfo.viewChangeTestPrimaryActionFlag>=(SystemInfo.node_num-1)*2/3){
                                SystemInfo.view=(SystemInfo.view+1)%SystemInfo.node_num;
                                SystemInfo.History=seg[4];
                                System.out.println("I am sleeping but I change my view");
                                SystemInfo.viewChangeTestPrimaryActionFlag=0;
                            }
                            return;
                        }
                        if (seg[2].equals("" + ((SystemInfo.view+1)%SystemInfo.node_num))) {
...
//--------------------------------CLIENT----------------------------------CLIENT----------------------------------------CLIENT--------------------------------------------CLIENT------------------------------------------
    class Client extends Thread {
        ...
        public Client(String message,int test_i) {
        ...
        public void run() {
            ...
            client_vc_timer.schedule(new ClientTimer(RequestMessage),4*1000,7*1000);// start a view change timer to  if (receive <2f+1 spec-response)//3*1000ms
            ...
            while (true) {
                ...
                if (SystemInfo.log_timeseal_SR.get(tx).size() >= ((node_num - 1) / 3 * 2 + 1)) {
                    ...
                    client_vc_timer.cancel();
                    client_commit_timer.schedule(new ClientCommitTimer(tx),3*1000,10*1000);//start a commit phase timer (receive 2f+1~3f spec-response)//2*1000ms
                    timer_vc_on=false;
                }
            ...
            Log.i("tx","tx = "+test_i+ ", delay = "+ (t_end-t));
        }
    ...
    class ClientCommitTimer extends TimerTask{
        ...
        public void run(){
            ArrayList<String> sr_list;
            while (true){
                sr_list=SystemInfo.log_timeseal_SR.get(timeseal);
                if (sr_list.size()!=0){
                    break;
                }
            }
    ...
    class RepFillHole extends Thread{
        ...
        public  void run(){
            ...
            */
            ...
            try {
                for (int i=0;i<=59;i++){// wait 3s for OR from primary
                    sleep(50);
            ...
            */
            ...
            if (OR_list.size()==0) {//primary case
                ...
                */
                ...
                //System.out.println("My timeseal log num is "+SystemInfo.timeSeal_log.size());
            }else{//rep case
            ...
            */
            ...
            while (true) {
                ...
                if (hate_list.size() >= (SystemInfo.node_num - 1) / 3 + 1) {
                    ...
                    String view_change = "<VIEW-CHANGE," + ((SystemInfo.view+1)%SystemInfo.node_num) + "," + CC + "," + OR + "," + SystemInfo.node_id + ">";
            ...
            if (SystemInfo.node_id==(SystemInfo.view+1)%SystemInfo.node_num){
                while (true) {
                    if (SystemInfo.isReceiving){
                        continue;
                    }
                    ...
                    if (vc_list.size() >= (SystemInfo.node_num - 1) * 2 / 3 + 1) {
                        ...
                        String new_view = "<NEW-VIEW," + ((SystemInfo.view+1)%SystemInfo.node_num) + "," + P + ">";
            ...
            while (true){
                ...
                if (nv_list.size()!=0){
                    ...
                    String view_confirm="<VIEW-CONFIRM,"+((SystemInfo.view+1)%SystemInfo.node_num)+","+(SystemInfo.request_id-1)+","+hn+","+SystemInfo.node_id+">";
